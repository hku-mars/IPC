...
namespace quickhull
{
    ...
    class QuickHull
    {
        using Vec3f = Vector3<T>;

        T m_epsilon, m_epsilonSquared, m_scale;
        bool m_planar;
        std::vector<Vec3f> m_planarPointCloudTemp;
        ...
        inline void setupInitialTetrahedron()
        {
            ...
            if (maxD == m_epsilon)
            {
                ...
                m_planar = true;
                const Vec3f N1 = mathutils::getTriangleNormal(baseTriangleVertices[1],
                                                             baseTriangleVertices[2],
                ...
                m_planarPointCloudTemp.insert(m_planarPointCloudTemp.begin(),
                                              m_vertexData.begin(),
                                              m_vertexData.end());
                const Vec3f extraPoint = N1 + m_vertexData[0];
        ...
        inline void createConvexHalfEdgeMesh()
        {
            ...
            while (!m_faceList.empty())
            {
                ...
                const Vec3f &activePoint = m_vertexData[tf.m_mostDistantPoint];
    ...
    public:
        ...
        inline ConvexHull<T> getConvexHull(const T *vertexData,
                                           ...
                                           T eps = defaultEps<T>())
        {
            VertexDataSource<T> vertexDataSource((const Vec3f *)vertexData, vertexCount);
            return getConvexHull(vertexDataSource, CCW, useOriginalIndices, eps);
        }
