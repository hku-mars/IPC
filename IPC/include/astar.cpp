...
bool AstarClass::SearchPath(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
    ...
    if (isOccupied(start_idx)) {
        ROS_ERROR("start point is in obstacle!");
        return false;
    }
    if (isOccupied(end_idx)) {
        ROS_ERROR("end point is in obstacle!");
        return false;
    }
    ...
    while (!open_lists_.empty()) {
        ...
        if (currentPtr->index == end_idx) {
            ...
            ROS_WARN("Time in A*  is %f ms, path cost if %f m", (ros::Time::now()-time_1).toSec()*1000.0, currentPtr->gScore*resolution_);
            return true;
        }
    ...
    if ((time_2 - time_1).toSec() > 0.01) {
        ROS_WARN("A* Can not find path. Time cost %f ms", (time_2 - time_1).toSec()*1000);
    }
    return false;
}
...
void AstarClass::GetPath(std::vector<Eigen::Vector3d> &path)
{
    ...
    // path[0] = ;
    // path[path.size()-1] = end_pt_;
}

void AstarClass::SimplifyPath(const std::vector<Eigen::Vector3d>& astar_path, 
...
void AstarClass::FloydHandle(const std::vector<Eigen::Vector3d>& astar_path, 
                                   std::vector<Eigen::Vector3d>& waypoint)
{
    waypoint.clear();
    ...
    for(int time = 0; time < 2; time++){
        for(int i = waypoint.size()-1; i > 0; i--){
            for(int j = 0; j < i-1; j++){
                if(CheckObstacle(waypoint[i], waypoint[j])){
                    for(int k = i-1; k > j; k--) {
...
void AstarClass::InsertPoints(std::vector<Eigen::Vector3d>& waypoint, double inter_dis)
{
    for (int i = 1; i < waypoint.size(); i++) {
        Eigen::Vector3d vector = waypoint[i] - waypoint[i-1];
        double dis = vector.norm();
        int inter_num = dis / inter_dis;
        for (int j = 1; j < inter_num; j++) {
            Eigen::Vector3d pt = waypoint[i-1] + vector / inter_num * j;
            waypoint.insert(waypoint.begin()+i+j-1, pt);
        }
    }
}
